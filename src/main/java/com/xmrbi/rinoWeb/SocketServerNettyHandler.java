...
public class SocketServerNettyHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {	// ChannelHandlerContext 오픈시 실행 ( 처음 접속시 )
        ...
        logger.trace("localPort =[{}]",localPort);
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws UnsupportedEncodingException {
        ...
        logger.debug("header Typ Cd = {}", new String(message.getHeaderTyp(), XWSConstants.CHAR_SET));
        logger.debug("header = {}", new String(message.getHeader(), XWSConstants.CHAR_SET));
        logger.debug("body = {}", new String(message.getBody(), XWSConstants.CHAR_SET));
